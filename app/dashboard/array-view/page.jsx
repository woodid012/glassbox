...
export default function ArrayViewPage() {
    ...
    // Pre-index inputs by groupId to avoid O(n) filters in render loops
    ...
    // Pre-index calculations by groupId
    ...
    // Pre-compute active groups (groups with inputs)
    ...
    const handleExportDebug = useCallback(async (exportMonthly = false) => {
        ...
        // Export Flags (F{id}) - ID-based references
        ...
        // Export Indexations (I{id}) - ID-based references
        ...
        <>
                            ...
                            <tr key={flag.id} className="hover:bg-slate-50 transition-colors border-b border-slate-200">
                                <td className="frozen-col w-[180px] min-w-[180px] border-r border-slate-200 px-2 py-1 pl-4">
                                    <div className="flex items-center gap-2">
                                        <span className="text-xs text-amber-600 bg-amber-100 px-1 py-0 rounded font-medium">F{refNum}</span>
                    ...
                    {Object.values(autoGeneratedIndexations).map((indexation) => {
                        ...
                        return (
                            <tr key={indexation.id} className="hover:bg-slate-50 transition-colors border-b border-slate-200">
                                <td className="frozen-col w-[180px] min-w-[180px] border-r border-slate-200 px-2 py-1 pl-4">
                                    <div className="flex items-center gap-2">
                                        <span className="text-xs text-cyan-600 bg-cyan-100 px-1 py-0 rounded font-medium">I{refNum}</span>
            ...
            {(() => {
                ...
                return activeInputGroups.map((group) => {
                    // Use pre-indexed inputs instead of filtering
            ...
            {modules.map((module, moduleIdx) => (
                <React.Fragment key={module.id}>
                    ...
                    {module.outputs && module.outputs.map((output, outputIdx) => {
                        // Get actual calculated output array from moduleOutputs
                        ...
                        return (
                            <tr key={outputLabel} className="hover:bg-slate-50 transition-colors border-b border-slate-100">
                                <td className="frozen-col w-[180px] min-w-[180px] border-r border-slate-200 px-2 py-0.5 pl-5">
                                    <div className="flex items-center gap-2">
                                        <span className="text-xs text-orange-500 bg-orange-50 px-1 py-0 rounded">
                                            M{moduleIdx + 1}.{outputIdx + 1}
                                        </span>
                                        <span className="text-[11px] text-slate-700">{(typeof outputLabel === 'string' ? outputLabel : '').replace(/_/g, ' ')}</span>
                                    </div>
                                </td>
                                {viewHeaders.map((header, colIndex) => {
                                    const isMonthlyView = viewMode === 'M'
