...
export function useReferenceMap({
    ...
    const flagRefs = useMemo(() => {
        ...
        Object.values(autoGeneratedFlags).forEach((flag) => {
            ...
            // Add period count variants (.M = months, .Q = quarters, .Y = years)
            if (flag.monthCountArray) refs[`F${refNum}.M`] = flag.monthCountArray
            if (flag.quarterCountArray) refs[`F${refNum}.Q`] = flag.quarterCountArray
            if (flag.yearCountArray) refs[`F${refNum}.Y`] = flag.yearCountArray
        })
        return refs
