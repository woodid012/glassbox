...
export function useInputArrays({
    ...
    const inputGlassArrays = useMemo(() => {
        ...
        inputGlass.forEach(input => {
            ...
            if (isConstantMode && groupValues.length > 0) {
                ...
                groupPeriods.forEach((period, periodIdx) => {
                    // Find matching index in model timeline
                    for (let t = 0; t < timeline.periods; t++) {
                        if (timeline.year[t] === period.year && timeline.month[t] === period.month) {
        ...
        return arrays
    }, [inputGlass, inputGlassGroups, timeline, config, keyPeriods])
