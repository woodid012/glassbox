...
export function useInputArrays({
    ...
    const inputType1Arrays = useMemo(() => {
        ...
        return arrays
    }, [inputType1, inputType1Groups, timeline, timelineLookup])
    ...
    const inputGlassArrays = useMemo(() => {
        ...
        inputGlass.forEach(input => {
            ...
            // Map group values to model timeline
            let arr = new Array(timeline.periods).fill(0)

            // For constant mode, use the value from getValuesArray and extend to full timeline
            ...
            if (isConstantMode && groupValues.length > 0) {
                ...
                // Just extend it to fill the entire model timeline
                const constantValue = groupValues[0] || 0
                arr.fill(constantValue)
            } else {
                // Non-constant modes: map group values to model timeline
                // Using O(1) Map lookup instead of O(n) loop per period
        ...
        return arrays
    }, [inputGlass, inputGlassGroups, timeline, config, keyPeriods, timelineLookup])
    ...
    const autoGeneratedFlags = useMemo(() => {
        ...
        return flags
    }, [inputType1, inputType1Groups, timeline, keyPeriods, timelineLookup])
